...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT
			...
			CAST(prd_startdate AS DATE) AS prd_startdate,
			CAST(
				LEAD(prd_startdate) OVER (PARTITION BY prd_key ORDER BY prd_startdate) 
				AS DATE
       ...
       -- PRINT '>> Load Duration: ' + CAST(DATEDIFF(SECOND, @start_time, @end_time) AS NVARCHAR) + ' seconds';
		...
		SELECT
			...
			CASE
				...
				ELSE cust_bdate
			END AS cust_bdate, -- Set future birthdates to NULL
			CASE
		...
		SELECT
			...
			CASE
				WHEN TRIM(cust_loc) = 'DE' THEN 'Germany'
				WHEN TRIM(cust_loc) IN ('US', 'USA') THEN 'United States'
				WHEN TRIM(cust_loc) = '' OR cust_loc IS NULL THEN 'n/a'
				ELSE TRIM(cust_loc)
